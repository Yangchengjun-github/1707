...
#define FW_VERSION_BMS 0x0002
#define HW_VERSION_BMS 0x0003
...
extern uint16_t  u16_nack_flag;
extern uint8_t RX_data [2]; // used in several functions to store data read from BQ769x2
...
extern uint8_t	OCC_Fault;  // over-current fault state
...
uint8_t BQ769x2_Init(void);
void BMS_I2C_Init(void);
void BQ769x2_CHG_OFF (void);
void BQ769x2_RESET_CHG_OFF (void) ;
void BQ769x2_DSG_OFF(void);
void BQ769x2_RESET_DSG_OFF(void);
...
void BQ769x2_ReadPFStatusB(void);
void BQ769x2_ReadPFStatusC(void);
uint16_t BQ769x2_ReadVoltage(uint8_t command);
void BQ769x2_Read_Vcells_123(void);
void BQ769x2_Read_Vcells_5(void);
void BQ769x2_Read_Vcells_7(void);
void BQ769x2_Read_Vcells_9(void);
...
extern uint8_t t_power_on_cnt_s;
...
typedef struct {
	uint8_t ovp_soft;//0
	uint8_t uvp_soft;
	uint8_t chg_ocp_soft;
	uint8_t dsg_ocp_soft;
	uint8_t chg_otp_soft;
	uint8_t chg_ltp_soft;
	uint8_t dsg_otp_soft;
	uint8_t dsg_ltp_soft;//7
	
	uint8_t mos_otp_soft;
	uint8_t mos_crash_soft;
	uint8_t ntc_error_soft;//10
	uint8_t bat_full_soft;//11
	uint8_t afe_nack20_soft;
	uint8_t bat_err_0v5_soft;
	uint8_t bat_err_ovp_soft;
	uint8_t bat_err_uvp_soft;
}bms_pro_flag_t;
typedef struct {
	uint16_t addr;
	uint16_t value;
...
extern bms_pro_flag_t bms_pro_flag;
extern uint16_t V_cells[6];
extern int16_t bms_curr;
extern uint16_t bms_tmp1,bms_tmp2,bms_tmp3,bms_tmp4;
extern uint16_t write_nack;
void	Flash_Read_Deal(void);
void save_ship_bat_pst(uint8_t bat_pst);
void write_error_long_flag(void);
void clear_error_long_flag(void);
//extern uint8_t long_error_flag;

extern uint16_t shutdown_bms_cnt;
extern uint16_t shutdown_bms_flag;
